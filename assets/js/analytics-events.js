/**
 * Ban.Tai Education Design 公式サイト - GA4 / KARTE イベント計測共通スクリプト
 * 
 * [概要]
 * サイト全体の主要なユーザーアクション（BOOTH、モニター登録、ライセンス申請、
 * 製品詳細、DBナビゲーション、お問い合わせ等）を計測します。
 * GA4 (Google Analytics 4) を正本とし、KARTE既存処理も維持します。
 * 
 * [固定仕様・プライバシーポリシー]
 * 1. GA4測定ID: G-KPGJ0R2KXR
 * 2. 公開カウンターは作成しない（GA4を正本とする）
 * 3. 本人除外（bantai_ga_disable）またはローカル環境（localhost, 127.0.0.1, file:）ではGA4送信を完全停止
 * 4. PII（個人識別情報）、URLクエリパラメータ、ハッシュ、mailtoアドレスはサニタイズ除去
 * 5. ファイルダウンロード（file_download）はGA4拡張計測に委ね、手動二重送信を行わない
 * 6. KARTE既存処理および教育計画PV計測を維持
 */
...
(function() {
  ...
  /**
   * GA4計測除外状態の判定
   * @returns {boolean} trueの場合はGA4イベント送信を停止
   */
  ...
  /**
   * URLおよびPIIのサニタイズ関数
   * クエリパラメータ、ハッシュ、mailto等の個人情報を除去
   * @param {string} rawUrl - サニタイズ対象URL
   * @returns {string} サニタイズされた安全なURL
   */
  ...
  /**
   * GA4カスタムイベント送信ヘルパー
   * @param {string} eventName - GA4イベント名
  ...
  /**
   * KARTEカスタムイベント送信ヘルパー
   * @param {string} eventName - KARTEイベント名
   * @param {Object} params - イベントパラメータ
   */
  function sendKarteEvent(eventName, params = {}) {
    if (typeof window.krt === 'function') {
      window.krt('send', eventName, params);
    }
  }
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    interactiveElements.forEach(element => {
      ...
      // --- 1. ダウンロードリンクの判定 ---
      // GA4拡張計測（file_download）に任せるため、GA4向けの手動file_download二重送信は行わない。
      // KARTEの既存 click_action のみ維持する。
      if (
        href.endsWith('.zip') ||
        href.endsWith('.exe') ||
        ...
        const itemName = `${appName} - 無料ダウンロード`;

        element.addEventListener('click', () => {
          sendKarteEvent('click_action', { item_name: itemName });
          // GA4へは手動file_downloadを重ねない（拡張計測の自動収集に任せる）
        });
      }
      ...
      // --- 2. BOOTH（販売・配布ページ）への遷移の判定 ---
        ...
        const appName = getAppNameFromContext(href, text);
        const itemName = `${appName} - BOOTH遷移`;

        element.addEventListener('click', () => {
          sendKarteEvent('click_action', { item_name: itemName });
          sendGA4Event('booth_click', {
            item_name: itemName,
      ...
      // --- 3. Google フォーム（モニター登録やライセンス申請など）への遷移の判定 ---
        ...
        const itemName = `${appName} - ${actionType}`;

        element.addEventListener('click', () => {
          sendKarteEvent('click_action', { item_name: itemName });
          sendGA4Event(ga4EventName, {
            item_name: itemName,
            product_name: appName,
            link_url: cleanHref
          });
      ...
      // --- 4. 製品詳細ページへの遷移判定（画像・パネルリンク） ---
        ...
        const appName = getAppNameFromContext(href, text);
        const itemName = `${appName} - 詳細ページ遷移`;

        element.addEventListener('click', () => {
          sendKarteEvent('click_action', { item_name: itemName });
          sendGA4Event('product_detail_click', {
            item_name: itemName,
            product_name: appName,
            link_url: cleanHref
          });
      ...
      // --- 5. データベース関連ナビゲーションの判定 ---
      else if (
        href.includes('/tools/school-database/') ||
        href.includes('/tools/university-database/') ||
        href.includes('/databases/')
      ) {
        let dbName = 'データベースポータル';
        if (href.includes('/tools/school-database/analytics')) {
          dbName = '全国学校データ・ダッシュボード';
        } else if (href.includes('/tools/school-database/')) {
          dbName = '全国学校データベース';
        } else if (href.includes('/tools/university-database/')) {
          dbName = '大学データベース';
        }

        element.addEventListener('click', () => {
          sendGA4Event('database_nav_click', {
      ...
      // --- 6. お問い合わせリンクの判定 ---
      else if (href.includes('/contact/') || href.startsWith('mailto:')) {
        element.addEventListener('click', () => {
          sendGA4Event('contact_click', {
            contact_type: href.startsWith('mailto:') ? 'email_client' : 'contact_page',
            link_url: cleanHref
          });
        });
      }
  ...
  document.addEventListener('DOMContentLoaded', () => {
    const sendEducationEvent = (eventName, params = {}) => {
      sendKarteEvent(eventName, params);
  ...
  // --- School DB analytics overlay（既存処理維持） ---
  (() => {
