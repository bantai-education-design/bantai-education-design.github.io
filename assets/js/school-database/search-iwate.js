// 岩手県学校宛先データベース - 検索・並び替え制御JS (assets/js/school-database/search-iwate.js)
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // 岩手県市町村順（五十音順、14市15町4村）
  const MUNICIPALITY_ORDER = [
    '一戸町', '一関市', '久慈市', '九戸村', '二戸市', '住田町',
    '八幡平市', '北上市', '大槌町', '大船渡市', '奥州市', '宮古市',
    '山田町', '岩手町', '岩泉町', '平泉町', '普代村', '洋野町',
    '滝沢市', '田野畑村', '盛岡市', '矢巾町', '紫波町', '花巻市',
    '葛巻町', '西和賀町', '軽米町', '遠野市', '野田村', '金ケ崎町',
    '釜石市', '陸前高田市', '雫石町'
  ];
  ...
  // 1. 岩手県データの読み込み
  fetch('/data/school-database/iwate.json')
    .then(response => {
      if (!response.ok) {
    ...
    .catch(error => {
      console.error('Error fetching Iwate school data:', error);
  ...
  function initCitySelect(data) {
    const availableCities = new Set(
      data.map(item => item.municipality).filter(c => c && c !== '岩手県')
    );
  ...
  function renderResults(results) {
    ...
    itemsToRender.forEach((school, index) => {
      ...
      const schoolId = `school-iwate-${index}`;
  ...
  if (downloadBtn) {
    downloadBtn.addEventListener('click', (e) => {
      ...
      trackEvent('school_csv', {
        'file_name': 'iwate_schools_address_filtered.csv',
        'results_count': currentFilteredResults.length
      });
    });
  }
  ...
  function downloadFilteredCSV(data) {
    ...
    link.setAttribute('download', `iwate_schools_address_filtered_${new Date().toISOString().slice(0, 10)}.csv`);
