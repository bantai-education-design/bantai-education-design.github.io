// 兵庫県学校データベース - 検索・並び替え制御JS (assets/js/school-database/search-hyogo.js)
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const MUNICIPALITY_ORDER = [
    'たつの市', '三木市', '三田市', '丹波市', '丹波篠山市', '伊丹市', '佐用郡佐用町', '加古川市', '加古郡播磨町', '加古郡稲美町', '加東市', '加西市', '南あわじ市', '多可郡多可町', '姫路市', '宍粟市', '宝塚市', '小野市', '尼崎市', '川西市', '川辺郡猪名川町', '揖保郡太子町', '明石市', '朝来市', '洲本市', '淡路市', '相生市', '神崎郡市川町', '神崎郡神河町', '神崎郡福崎町', '神戸市', '美方郡新温泉町', '美方郡香美町', '芦屋市', '西宮市', '西脇市', '豊岡市', '赤穂市', '赤穂郡上郡町', '養父市', '高砂市'
  ];
  ...
  // 1. 兵庫県データの読み込み
  fetch('/data/school-database/hyogo.json')
    .then(response => {
      if (!response.ok) {
  ...
  function initCitySelect(data) {
    const availableCities = new Set(
      data.map(item => item.municipality).filter(c => c && c !== '兵庫県')
    );
  ...
  function renderResults(results) {
    ...
    itemsToRender.forEach((school, index) => {
      ...
      const schoolId = `school-hyogo-${index}`;
  ...
  if (downloadBtn) {
    downloadBtn.addEventListener('click', (e) => {
      ...
      trackEvent('school_csv', {
        'file_name': 'hyogo_schools_address_filtered.csv',
        'results_count': currentFilteredResults.length
      });
    });
  }
  ...
  function downloadFilteredCSV(data) {
    ...
    link.setAttribute('download', `hyogo_schools_address_filtered_${new Date().toISOString().slice(0, 10)}.csv`);
