// 愛媛県学校データベース - 検索・並び替え制御JS (assets/js/school-database/search-ehime.js)
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const MUNICIPALITY_ORDER = [
    '上浮穴郡久万高原町', '今治市', '伊予市', '伊予郡松前町', '伊予郡砥部町', '八幡浜市', '北宇和郡松野町', '北宇和郡鬼北町', '南宇和郡愛南町', '喜多郡内子町', '四国中央市', '大洲市', '宇和島市', '新居浜市', '東温市', '松山市', '松山市上市', '松山市市', '西予市', '西宇和郡伊方町', '西条市', '西条市朔日市', '越智郡上島町'
  ];
  ...
  // 1. 愛媛県データの読み込み
  fetch('/data/school-database/ehime.json')
    .then(response => {
      if (!response.ok) {
  ...
  function initCitySelect(data) {
    const availableCities = new Set(
      data.map(item => item.municipality).filter(c => c && c !== '愛媛県')
    );
  ...
  function renderResults(results) {
    ...
    itemsToRender.forEach((school, index) => {
      ...
      const schoolId = `school-ehime-${index}`;
  ...
  if (downloadBtn) {
    downloadBtn.addEventListener('click', (e) => {
      ...
      trackEvent('school_csv', {
        'file_name': 'ehime_schools_address_filtered.csv',
        'results_count': currentFilteredResults.length
      });
    });
  }
  ...
  function downloadFilteredCSV(data) {
    ...
    link.setAttribute('download', `ehime_schools_address_filtered_${new Date().toISOString().slice(0, 10)}.csv`);
