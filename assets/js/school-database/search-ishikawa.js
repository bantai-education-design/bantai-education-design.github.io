// 石川県学校データベース - 検索・並び替え制御JS (assets/js/school-database/search-ishikawa.js)
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const MUNICIPALITY_ORDER = [
    ...
    '羽咋市', 'かほく市', '白山市', '能美市', '野々市市', '能美郡川北町',
    '河北郡津幡町', '河北郡内灘町', '羽咋郡志賀町', '羽咋郡宝達志水町', '鹿島郡中能登町', '鳳珠郡穴水町',
    '鳳珠郡能登町'
  ];
  ...
  // 1. 石川県データの読み込み
  fetch('/data/school-database/ishikawa.json')
    .then(response => {
      if (!response.ok) {
    ...
    .catch(error => {
      console.error('Error fetching Ishikawa school data:', error);
  ...
  function initCitySelect(data) {
    const availableCities = new Set(
      data.map(item => item.municipality).filter(c => c && c !== '石川県')
    );
  ...
  function renderResults(results) {
    ...
    itemsToRender.forEach((school, index) => {
      ...
      const schoolId = `school-ishikawa-${index}`;
  ...
  if (downloadBtn) {
    downloadBtn.addEventListener('click', (e) => {
      ...
      trackEvent('school_csv', {
        'file_name': 'ishikawa_schools_address_filtered.csv',
        'results_count': currentFilteredResults.length
      });
    });
  }
  ...
  function downloadFilteredCSV(data) {
    ...
    link.setAttribute('download', `ishikawa_schools_address_filtered_${new Date().toISOString().slice(0, 10)}.csv`);
