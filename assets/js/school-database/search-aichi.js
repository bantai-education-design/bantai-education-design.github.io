// 愛知県学校宛先データベース - 検索・並び替え制御JS (assets/js/school-database/search-aichi.js)
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // 1. 愛知県データの読み込み
  fetch('/data/school-database/aichi.json')
    .then(response => {
      if (!response.ok) {
    ...
    .catch(error => {
      console.error('Error fetching Aichi school data:', error);
  ...
  function initCitySelect(data) {
    const availableCities = new Set(
      data.map(item => item.municipality).filter(c => c && c !== '愛知県')
    );
  ...
  function renderResults(results) {
    ...
    itemsToRender.forEach((school, index) => {
      ...
      const schoolId = `school-aichi-${index}`;
  ...
  if (downloadBtn) {
    downloadBtn.addEventListener('click', (e) => {
      ...
      trackEvent('school_csv', {
        'file_name': 'aichi_schools_address_filtered.csv',
        'results_count': currentFilteredResults.length
      });
    });
  }
  ...
  function downloadFilteredCSV(data) {
    ...
    link.setAttribute('download', `aichi_schools_address_filtered_${new Date().toISOString().slice(0, 10)}.csv`);
