// 青森県学校データベース - 検索・並び替え制御JS (assets/js/school-database/search-aomori.js)
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // 1. 青森県データの読み込み
  fetch('/data/school-database/aomori.json')
    .then(response => {
      if (!response.ok) {
  ...
  function initCitySelect(data) {
    const availableCities = new Set(
      data.map(item => item.municipality).filter(c => c && c !== '青森県')
    );
  ...
  function renderResults(results) {
    ...
    itemsToRender.forEach((school, index) => {
      ...
      const schoolId = `school-aomori-${index}`;
  ...
  if (downloadBtn) {
    downloadBtn.addEventListener('click', (e) => {
      ...
      trackEvent('school_csv', {
        'file_name': 'aomori_schools_address_filtered.csv',
        'results_count': currentFilteredResults.length
      });
    });
  }
  ...
  function downloadFilteredCSV(data) {
    ...
    link.setAttribute('download', `aomori_schools_address_filtered_${new Date().toISOString().slice(0, 10)}.csv`);
