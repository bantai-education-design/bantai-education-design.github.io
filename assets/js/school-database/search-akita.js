// 秋田県学校データベース - 検索・並び替え制御JS (assets/js/school-database/search-akita.js)
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // 1. 秋田県データの読み込み
  fetch('/data/school-database/akita.json')
    .then(response => {
      if (!response.ok) {
  ...
  function initCitySelect(data) {
    const availableCities = new Set(
      data.map(item => item.municipality).filter(c => c && c !== '秋田県')
    );
  ...
  function renderResults(results) {
    ...
    itemsToRender.forEach((school, index) => {
      ...
      const schoolId = `school-akita-${index}`;
  ...
  if (downloadBtn) {
    downloadBtn.addEventListener('click', (e) => {
      ...
      trackEvent('school_csv', {
        'file_name': 'akita_schools_address_filtered.csv',
        'results_count': currentFilteredResults.length
      });
    });
  }
  ...
  function downloadFilteredCSV(data) {
    ...
    link.setAttribute('download', `akita_schools_address_filtered_${new Date().toISOString().slice(0, 10)}.csv`);
