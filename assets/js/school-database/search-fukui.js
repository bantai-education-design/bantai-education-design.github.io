// 福井県学校データベース - 検索・並び替え制御JS (assets/js/school-database/search-fukui.js)
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // 1. 福井県データの読み込み
  fetch('/data/school-database/fukui.json')
    .then(response => {
      if (!response.ok) {
  ...
  function initCitySelect(data) {
    const availableCities = new Set(
      data.map(item => item.municipality).filter(c => c && c !== '福井県')
    );
  ...
  function renderResults(results) {
    ...
    itemsToRender.forEach((school, index) => {
      ...
      const schoolId = `school-fukui-${index}`;
  ...
  if (downloadBtn) {
    downloadBtn.addEventListener('click', (e) => {
      ...
      trackEvent('school_csv', {
        'file_name': 'fukui_schools_address_filtered.csv',
        'results_count': currentFilteredResults.length
      });
    });
  }
  ...
  function downloadFilteredCSV(data) {
    ...
    link.setAttribute('download', `fukui_schools_address_filtered_${new Date().toISOString().slice(0, 10)}.csv`);
