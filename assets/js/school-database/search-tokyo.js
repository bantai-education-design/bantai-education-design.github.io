// 東京都学校データベース - 検索・並び替え制御JS (assets/js/school-database/search-tokyo.js)
// data/school-database/tokyo.json（現行の全件データ）を使用する。
// 旧 /tools/tokyo-school-address/search.js（別の古いデータファイル、
// 公立小中学校中心）はレガシーであり、このスクリプトはそれを置き換える。
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // 1. 東京都データの読み込み（現行データ: data/school-database/tokyo.json）
  fetch('/data/school-database/tokyo.json')
    .then(response => {
      if (!response.ok) {
    ...
    .catch(error => {
      console.error('Error fetching Tokyo school data:', error);
  ...
  function renderResults(results) {
    ...
    itemsToRender.forEach((school, index) => {
      ...
      const schoolId = `school-tokyo-${index}`;
