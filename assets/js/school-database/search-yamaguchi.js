// 山口県学校データベース - 検索・並び替え制御JS (assets/js/school-database/search-yamaguchi.js)
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const MUNICIPALITY_ORDER = [
    '下松市', '下関市', '光市', '周南市', '大島郡周防大島町', '宇部市', '山口市', '山陽小野田市', '岩国市', '柳井市', '熊毛郡上関町', '熊毛郡平生町', '熊毛郡田布施町', '玖珂郡和木町', '美祢市', '萩市', '長門市', '防府市', '阿武郡阿武町'
  ];
  ...
  // 1. 山口県データの読み込み
  fetch('/data/school-database/yamaguchi.json')
    .then(response => {
      if (!response.ok) {
  ...
  function initCitySelect(data) {
    const availableCities = new Set(
      data.map(item => item.municipality).filter(c => c && c !== '山口県')
    );
  ...
  function renderResults(results) {
    ...
    itemsToRender.forEach((school, index) => {
      ...
      const schoolId = `school-yamaguchi-${index}`;
  ...
  if (downloadBtn) {
    downloadBtn.addEventListener('click', (e) => {
      ...
      trackEvent('school_csv', {
        'file_name': 'yamaguchi_schools_address_filtered.csv',
        'results_count': currentFilteredResults.length
      });
    });
  }
  ...
  function downloadFilteredCSV(data) {
    ...
    link.setAttribute('download', `yamaguchi_schools_address_filtered_${new Date().toISOString().slice(0, 10)}.csv`);
