// 福岡県学校宛先データベース - 検索・並び替え制御JS (assets/js/school-database/search-fukuoka.js)
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // 1. 福岡県データの読み込み
  fetch('/data/school-database/fukuoka.json')
    .then(response => {
      if (!response.ok) {
    ...
    .catch(error => {
      console.error('Error fetching Fukuoka school data:', error);
  ...
  function initCitySelect(data) {
    const availableCities = new Set(
      data.map(item => item.municipality).filter(c => c && c !== '福岡県')
    );
  ...
  function renderResults(results) {
    ...
    itemsToRender.forEach((school, index) => {
      ...
      const schoolId = `school-fukuoka-${index}`;
  ...
  if (downloadBtn) {
    downloadBtn.addEventListener('click', (e) => {
      ...
      trackEvent('school_csv', {
        'file_name': 'fukuoka_schools_address_filtered.csv',
        'results_count': currentFilteredResults.length
      });
    });
  }
  ...
  function downloadFilteredCSV(data) {
    ...
    link.setAttribute('download', `fukuoka_schools_address_filtered_${new Date().toISOString().slice(0, 10)}.csv`);
