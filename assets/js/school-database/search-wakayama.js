// 和歌山県学校データベース - 検索・並び替え制御JS (assets/js/school-database/search-wakayama.js)
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // 1. 和歌山県データの読み込み
  fetch('/data/school-database/wakayama.json')
    .then(response => {
      if (!response.ok) {
    ...
    .catch(error => {
      console.error('Error fetching 和歌山県 school data:', error);
  ...
  function initCitySelect(data) {
    const availableCities = new Set(
      data.map(item => item.municipality).filter(c => c && c !== '和歌山県')
    );
  ...
  function renderResults(results) {
    ...
    itemsToRender.forEach((school, index) => {
      ...
      const schoolId = `school-wakayama-${index}`;
  ...
  if (downloadBtn) {
    downloadBtn.addEventListener('click', (e) => {
      ...
      trackEvent('school_csv', {
        'file_name': 'wakayama_schools_address_filtered.csv',
        'results_count': currentFilteredResults.length
      });
    });
  }
  ...
  function downloadFilteredCSV(data) {
    ...
    link.setAttribute('download', `wakayama_schools_address_filtered_${new Date().toISOString().slice(0, 10)}.csv`);
