// 三重県学校データベース - 検索・並び替え制御JS (assets/js/school-database/search-mie.js)
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const MUNICIPALITY_ORDER = [
    ...
    '志摩市', '伊賀市', '桑名郡木曽岬町', '員弁郡東員町', '三重郡菰野町', '三重郡朝日町',
    '三重郡川越町', '多気郡多気町', '多気郡明和町', '多気郡大台町', '度会郡玉城町', '度会郡度会町',
    '度会郡大紀町', '度会郡南伊勢町', '北牟婁郡紀北町', '南牟婁郡御浜町', '南牟婁郡紀宝町'
  ];
  ...
  // 1. 三重県データの読み込み
  fetch('/data/school-database/mie.json')
    .then(response => {
      if (!response.ok) {
  ...
  function initCitySelect(data) {
    const availableCities = new Set(
      data.map(item => item.municipality).filter(c => c && c !== '三重県')
    );
  ...
  function renderResults(results) {
    ...
    itemsToRender.forEach((school, index) => {
      ...
      const schoolId = `school-mie-${index}`;
  ...
  if (downloadBtn) {
    downloadBtn.addEventListener('click', (e) => {
      ...
      trackEvent('school_csv', {
        'file_name': 'mie_schools_address_filtered.csv',
        'results_count': currentFilteredResults.length
      });
    });
  }
  ...
  function downloadFilteredCSV(data) {
    ...
    link.setAttribute('download', `mie_schools_address_filtered_${new Date().toISOString().slice(0, 10)}.csv`);
