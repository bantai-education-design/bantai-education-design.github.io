// 福島県学校宛先データベース - 検索・並び替え制御JS (assets/js/school-database/search-fukushima.js)
// フィールド名は千葉県版・神奈川県版と同じ（name, establishment, course は配列）。
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // 1. 福島県データの読み込み
  fetch('/data/school-database/fukushima.json')
    .then(response => {
      if (!response.ok) {
    ...
    .catch(error => {
      console.error('Error fetching Fukushima school data:', error);
  ...
  function initCitySelect(data) {
    const availableCities = new Set(
      data.map(item => item.municipality).filter(c => c && c !== '福島県')
    );
  ...
  function renderResults(results) {
    ...
    itemsToRender.forEach((school, index) => {
      ...
      const schoolId = `school-fukushima-${index}`;
  ...
  if (downloadBtn) {
    downloadBtn.addEventListener('click', (e) => {
      ...
      trackEvent('school_csv', {
        'file_name': 'fukushima_schools_address_filtered.csv',
        'results_count': currentFilteredResults.length
      });
    });
  }
  ...
  function downloadFilteredCSV(data) {
    ...
    link.setAttribute('download', `fukushima_schools_address_filtered_${new Date().toISOString().slice(0, 10)}.csv`);
