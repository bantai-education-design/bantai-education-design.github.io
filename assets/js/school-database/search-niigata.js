// 新潟県学校宛先データベース - 検索・並び替え制御JS (assets/js/school-database/search-niigata.js)
// フィールド名は福島県版・宮城県版と同じ（name, establishment, course は配列）。
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // 新潟県行政順（新潟市の区 -> 上越教育事務所管内 -> 中越教育事務所管内 ->
  ...
  // 1. 新潟県データの読み込み
  fetch('/data/school-database/niigata.json')
    .then(response => {
      if (!response.ok) {
    ...
    .catch(error => {
      console.error('Error fetching Niigata school data:', error);
  ...
  function initCitySelect(data) {
    const availableCities = new Set(
      data.map(item => item.municipality).filter(c => c && c !== '新潟県')
    );
  ...
  function renderResults(results) {
    ...
    itemsToRender.forEach((school, index) => {
      ...
      const schoolId = `school-niigata-${index}`;
  ...
  if (downloadBtn) {
    downloadBtn.addEventListener('click', (e) => {
      ...
      trackEvent('school_csv', {
        'file_name': 'niigata_schools_address_filtered.csv',
        'results_count': currentFilteredResults.length
      });
    });
  }
  ...
  function downloadFilteredCSV(data) {
    ...
    link.setAttribute('download', `niigata_schools_address_filtered_${new Date().toISOString().slice(0, 10)}.csv`);
