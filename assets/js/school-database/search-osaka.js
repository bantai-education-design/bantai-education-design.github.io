...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // 大阪府市町村順（大阪市24区・堺市7区は行政区順、他39市町村は五十音順）
  const MUNICIPALITY_ORDER = [
    ...
    '大阪市西成区', '大阪市住之江区', '大阪市鶴見区', '大阪市平野区',
    // 堺市7区
    '堺市堺区', '堺市中区', '堺市東区', '堺市西区', '堺市南区', '堺市北区', '堺市美原区',
    // その他市町村(五十音順)
    '和泉市', '泉大津市', '泉佐野市', '池田市', '茨木市', '大阪狭山市',
    '貝塚市', '柏原市', '交野市', '門真市', '河南町', '河内長野市', '岸和田市', '熊取町',
    '四條畷市', '島本町', '吹田市', '摂津市', '泉南市',
    '太子町', '高石市', '高槻市', '忠岡町', '田尻町', '大東市', '千早赤阪村', '豊中市', '豊能町', '富田林市',
    '寝屋川市', '能勢町',
  ...
  // 1. 大阪府データの読み込み
  fetch('/data/school-database/osaka.json')
    .then(response => {
      if (!response.ok) {
    ...
    .catch(error => {
      console.error('Error fetching Osaka school data:', error);
  ...
  function initCitySelect(data) {
    const availableCities = new Set(
      data.map(item => item.municipality).filter(c => c && c !== '大阪府')
    );
  ...
  function renderResults(results) {
    ...
    itemsToRender.forEach((school, index) => {
      ...
      const schoolId = `school-osaka-${index}`;
  ...
  if (downloadBtn) {
    downloadBtn.addEventListener('click', (e) => {
      ...
      trackEvent('school_csv', {
        'file_name': 'osaka_schools_address_filtered.csv',
        'results_count': currentFilteredResults.length
      });
    });
  }
  ...
  function downloadFilteredCSV(data) {
    ...
    link.setAttribute('download', `osaka_schools_address_filtered_${new Date().toISOString().slice(0, 10)}.csv`);
