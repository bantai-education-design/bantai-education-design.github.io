// 山形県学校データベース - 検索・並び替え制御JS (assets/js/school-database/search-kumamoto.js)
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // 1. 山形県データの読み込み
  fetch('/data/school-database/kumamoto.json')
    .then(response => {
      if (!response.ok) {
  ...
  function renderResults(results) {
    ...
    itemsToRender.forEach((school, index) => {
      ...
      const schoolId = `school-kumamoto-${index}`;
  ...
  if (downloadBtn) {
    downloadBtn.addEventListener('click', (e) => {
      ...
      trackEvent('school_csv', {
        'file_name': 'kumamoto_schools_address_filtered.csv',
        'results_count': currentFilteredResults.length
      });
    });
  }
  ...
  function downloadFilteredCSV(data) {
    ...
    link.setAttribute('download', `kumamoto_schools_address_filtered_${new Date().toISOString().slice(0, 10)}.csv`);
