// 鳥取県学校データベース - 検索・並び替え制御JS (assets/js/school-database/search-tottori.js)
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const MUNICIPALITY_ORDER = [
    '鳥取市', '米子市', '倉吉市', '境港市', '岩美郡岩美町', '八頭郡若桜町',
    '八頭郡智頭町', '八頭郡八頭町', '東伯郡三朝町', '東伯郡湯梨浜町', '東伯郡琴浦町', '東伯郡北栄町',
    '西伯郡日吉津村', '西伯郡大山町', '西伯郡南部町', '西伯郡伯耆町', '日野郡日南町', '日野郡日野町',
    '日野郡江府町'
  ];
  ...
  // 1. 鳥取県データの読み込み
  fetch('/data/school-database/tottori.json')
    .then(response => {
      if (!response.ok) {
  ...
  function initCitySelect(data) {
    const availableCities = new Set(
      data.map(item => item.municipality).filter(c => c && c !== '鳥取県')
    );
  ...
  function renderResults(results) {
    ...
    itemsToRender.forEach((school, index) => {
      ...
      const schoolId = `school-tottori-${index}`;
  ...
  if (downloadBtn) {
    downloadBtn.addEventListener('click', (e) => {
      ...
      trackEvent('school_csv', {
        'file_name': 'tottori_schools_address_filtered.csv',
        'results_count': currentFilteredResults.length
      });
    });
  }
  ...
  function downloadFilteredCSV(data) {
    ...
    link.setAttribute('download', `tottori_schools_address_filtered_${new Date().toISOString().slice(0, 10)}.csv`);
