// 京都府学校データベース - 検索・並び替え制御JS (assets/js/school-database/search-kyoto.js)
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const MUNICIPALITY_ORDER = [
    ...
    '木津川市', '乙訓郡大山崎町', '久世郡久御山町', '綴喜郡井手町', '綴喜郡宇治田原町', '相楽郡笠置町',
    '相楽郡和束町', '相楽郡精華町', '相楽郡南山城村', '船井郡京丹波町', '与謝郡伊根町', '与謝郡与謝野町'
  ];
  ...
  // 1. 京都府データの読み込み
  fetch('/data/school-database/kyoto.json')
    .then(response => {
      if (!response.ok) {
    ...
    .catch(error => {
      console.error('Error fetching Kyoto school data:', error);
  ...
  function initCitySelect(data) {
    const availableCities = new Set(
      data.map(item => item.municipality).filter(c => c && c !== '京都府')
    );
  ...
  function renderResults(results) {
    ...
    itemsToRender.forEach((school, index) => {
      ...
      const schoolId = `school-kyoto-${index}`;
  ...
  if (downloadBtn) {
    downloadBtn.addEventListener('click', (e) => {
      ...
      trackEvent('school_csv', {
        'file_name': 'kyoto_schools_address_filtered.csv',
        'results_count': currentFilteredResults.length
      });
    });
  }
  ...
  function downloadFilteredCSV(data) {
    ...
    link.setAttribute('download', `kyoto_schools_address_filtered_${new Date().toISOString().slice(0, 10)}.csv`);
