// 山形県学校データベース - 検索・並び替え制御JS (assets/js/school-database/search-nagasaki.js)
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // 1. 山形県データの読み込み
  fetch('/data/school-database/nagasaki.json')
    .then(response => {
      if (!response.ok) {
  ...
  function renderResults(results) {
    ...
    itemsToRender.forEach((school, index) => {
      ...
      const schoolId = `school-nagasaki-${index}`;
  ...
  if (downloadBtn) {
    downloadBtn.addEventListener('click', (e) => {
      ...
      trackEvent('school_csv', {
        'file_name': 'nagasaki_schools_address_filtered.csv',
        'results_count': currentFilteredResults.length
      });
    });
  }
  ...
  function downloadFilteredCSV(data) {
    ...
    link.setAttribute('download', `nagasaki_schools_address_filtered_${new Date().toISOString().slice(0, 10)}.csv`);
