// 滋賀県学校データベース - 検索・並び替え制御JS (assets/js/school-database/search-shiga.js)
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // 1. 滋賀県データの読み込み
  fetch('/data/school-database/shiga.json')
    .then(response => {
      if (!response.ok) {
    ...
    .catch(error => {
      console.error('Error fetching Shiga school data:', error);
  ...
  function initCitySelect(data) {
    const availableCities = new Set(
      data.map(item => item.municipality).filter(c => c && c !== '滋賀県')
    );
  ...
  function renderResults(results) {
    ...
    itemsToRender.forEach((school, index) => {
      ...
      const schoolId = `school-shiga-${index}`;
  ...
  if (downloadBtn) {
    downloadBtn.addEventListener('click', (e) => {
      ...
      trackEvent('school_csv', {
        'file_name': 'shiga_schools_address_filtered.csv',
        'results_count': currentFilteredResults.length
      });
    });
  }
  ...
  function downloadFilteredCSV(data) {
    ...
    link.setAttribute('download', `shiga_schools_address_filtered_${new Date().toISOString().slice(0, 10)}.csv`);
