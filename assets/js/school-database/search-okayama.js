// 岡山県学校データベース - 検索・並び替え制御JS (assets/js/school-database/search-okayama.js)
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const MUNICIPALITY_ORDER = [
    '久米郡久米南町', '久米郡美咲町', '井原市', '倉敷市', '備前市', '加賀郡吉備中央町', '勝田郡勝央町', '勝田郡奈義町', '和気郡和気町', '小田郡矢掛町', '岡山市', '新見市', '津山市', '浅口市', '浅口郡里庄町', '瀬戸内市', '玉野市', '真庭市', '真庭郡新庄村', '笠岡市', '総社市', '美作市', '苫田郡鏡野町', '英田郡西粟倉村', '赤磐市', '都窪郡早島町', '高梁市'
  ];
  ...
  // 1. 岡山県データの読み込み
  fetch('/data/school-database/okayama.json')
    .then(response => {
      if (!response.ok) {
  ...
  function initCitySelect(data) {
    const availableCities = new Set(
      data.map(item => item.municipality).filter(c => c && c !== '岡山県')
    );
  ...
  function renderResults(results) {
    ...
    itemsToRender.forEach((school, index) => {
      ...
      const schoolId = `school-okayama-${index}`;
  ...
  if (downloadBtn) {
    downloadBtn.addEventListener('click', (e) => {
      ...
      trackEvent('school_csv', {
        'file_name': 'okayama_schools_address_filtered.csv',
        'results_count': currentFilteredResults.length
      });
    });
  }
  ...
  function downloadFilteredCSV(data) {
    ...
    link.setAttribute('download', `okayama_schools_address_filtered_${new Date().toISOString().slice(0, 10)}.csv`);
