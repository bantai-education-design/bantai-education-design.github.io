...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // 神奈川県行政順（横浜市各区 -> 川崎市各区 -> 相模原市各区 -> 市部 -> 郡部）
  ...
  // 1. 神奈川県データの読み込み
  fetch('/data/school-database/kanagawa.json')
    .then(response => {
      if (!response.ok) {
    ...
    .catch(error => {
      console.error('Error fetching Kanagawa school data:', error);
  ...
  function initCitySelect(data) {
    const availableCities = new Set(
      data.map(item => item.municipality).filter(c => c && c !== '神奈川県')
    );
  ...
  function renderResults(results) {
    ...
    itemsToRender.forEach((school, index) => {
      ...
      const schoolId = `school-kanagawa-${index}`;
  ...
  if (downloadBtn) {
    downloadBtn.addEventListener('click', (e) => {
      ...
      trackEvent('school_csv', {
        'file_name': 'kanagawa_schools_address_filtered.csv',
        'results_count': currentFilteredResults.length
      });
    });
  }
  ...
  function downloadFilteredCSV(data) {
    ...
    link.setAttribute('download', `kanagawa_schools_address_filtered_${new Date().toISOString().slice(0, 10)}.csv`);
