// 岐阜県学校データベース - 検索・並び替え制御JS (assets/js/school-database/search-gifu.js)
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const MUNICIPALITY_ORDER = [
    ...
    '郡上市', '下呂市', '海津市', '羽島郡岐南町', '羽島郡笠松町', '養老郡養老町',
    '不破郡垂井町', '不破郡関ケ原町', '安八郡神戸町', '安八郡輪之内町', '安八郡安八町', '揖斐郡揖斐川町',
    '揖斐郡大野町', '揖斐郡池田町', '本巣郡北方町', '加茂郡坂祝町', '加茂郡富加町', '加茂郡川辺町',
    '加茂郡七宗町', '加茂郡八百津町', '加茂郡白川町', '加茂郡東白川村', '可児郡御嵩町', '大野郡白川村'
  ];
  ...
  // 1. 岐阜県データの読み込み
  fetch('/data/school-database/gifu.json')
    .then(response => {
      if (!response.ok) {
  ...
  function initCitySelect(data) {
    const availableCities = new Set(
      data.map(item => item.municipality).filter(c => c && c !== '岐阜県')
    );
  ...
  function renderResults(results) {
    ...
    itemsToRender.forEach((school, index) => {
      ...
      const schoolId = `school-gifu-${index}`;
  ...
  if (downloadBtn) {
    downloadBtn.addEventListener('click', (e) => {
      ...
      trackEvent('school_csv', {
        'file_name': 'gifu_schools_address_filtered.csv',
        'results_count': currentFilteredResults.length
      });
    });
  }
  ...
  function downloadFilteredCSV(data) {
    ...
    link.setAttribute('download', `gifu_schools_address_filtered_${new Date().toISOString().slice(0, 10)}.csv`);
