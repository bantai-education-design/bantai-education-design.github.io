// 高知県学校データベース - 検索・並び替え制御JS (assets/js/school-database/search-kochi.js)
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const MUNICIPALITY_ORDER = [
    '南国市', '吾川郡いの町', '吾川郡仁淀川町', '四万十市', '四万十市下田魚市', '土佐市', '土佐清水市', '土佐郡土佐町', '土佐郡大川村', '安芸市', '安芸郡北川村', '安芸郡奈半利町', '安芸郡安田町', '安芸郡東洋町', '安芸郡田野町', '安芸郡芸西村', '安芸郡馬路村', '室戸市', '宿毛市', '宿毛市与市', '幡多郡三原村', '幡多郡大月町', '幡多郡黒潮町', '長岡郡大豊町', '長岡郡本山町', '須崎市', '須崎市東古市', '香南市', '香南市野市', '香美市', '高岡郡中土佐町', '高岡郡佐川町', '高岡郡四万十町', '高岡郡日高村', '高岡郡梼原町', '高岡郡檮原町', '高岡郡津野町', '高岡郡越知町', '高知市'
  ];
  ...
  // 1. 高知県データの読み込み
  fetch('/data/school-database/kochi.json')
    .then(response => {
      if (!response.ok) {
  ...
  function initCitySelect(data) {
    const availableCities = new Set(
      data.map(item => item.municipality).filter(c => c && c !== '高知県')
    );
  ...
  function renderResults(results) {
    ...
    itemsToRender.forEach((school, index) => {
      ...
      const schoolId = `school-kochi-${index}`;
  ...
  if (downloadBtn) {
    downloadBtn.addEventListener('click', (e) => {
      ...
      trackEvent('school_csv', {
        'file_name': 'kochi_schools_address_filtered.csv',
        'results_count': currentFilteredResults.length
      });
    });
  }
  ...
  function downloadFilteredCSV(data) {
    ...
    link.setAttribute('download', `kochi_schools_address_filtered_${new Date().toISOString().slice(0, 10)}.csv`);
