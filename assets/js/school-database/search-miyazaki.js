// 宮崎県学校データベース - 検索・並び替え制御JS (assets/js/school-database/search-miyazaki.js)
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // 宮崎県の市町村順（行政表示順に近い並び）
  const MUNICIPALITY_ORDER = ["えびの市","串間市","串間市大字市","宮崎市","小林市","延岡市","日南市","日向市","西都市","都城市","都城市南横市","児湯郡川南町","児湯郡新富町","児湯郡木城町","児湯郡西米良村","児湯郡都農町","児湯郡高鍋町","北諸県郡三股町","東臼杵郡椎葉村","東臼杵郡美郷町","東臼杵郡諸塚村","東臼杵郡門川町","東諸県郡国富町","東諸県郡綾町","西臼杵郡五ヶ瀬町","西臼杵郡日之影町","西臼杵郡高千穂町","西諸県郡高原町"];
  ...
  // 1. 宮崎県データの読み込み
  fetch('/data/school-database/miyazaki.json')
    .then(response => {
      if (!response.ok) {
    ...
    .catch(error => {
      console.error('Error fetching 宮崎県 school data:', error);
  ...
  function initCitySelect(data) {
    const availableCities = new Set(
      data.map(item => item.municipality).filter(c => c && c !== '宮崎県')
    );
  ...
  function renderResults(results) {
    ...
    itemsToRender.forEach((school, index) => {
      ...
      const schoolId = `school-miyazaki-${index}`;
  ...
  if (downloadBtn) {
    downloadBtn.addEventListener('click', (e) => {
      ...
      trackEvent('school_csv', {
        'file_name': 'miyazaki_schools_address_filtered.csv',
        'results_count': currentFilteredResults.length
      });
    });
  }
  ...
  function downloadFilteredCSV(data) {
    ...
    link.setAttribute('download', `miyazaki_schools_address_filtered_${new Date().toISOString().slice(0, 10)}.csv`);
