...
try:
    with sync_playwright() as p:
        ...
        time.sleep(1)
        hp_path = os.path.join(out_dir, "weekly_ai_news_homepage.png")
        ...
        time.sleep(1)
        cl_path = os.path.join(out_dir, "weekly_ai_news_columns_list.png")
        page.screenshot(path=cl_path)
        print(f"Columns list screenshot saved to {cl_path}", flush=True)
        
        # Open Vol 7 modal directly via JS
        page.evaluate("openColumnModal('column-weekly-ai-news-20260918')")
        page.wait_for_selector("#column-modal.is-open")
        time.sleep(1)
        
        # Top of modal screenshot
        modal_dialog = page.locator(".column-modal-dialog")
        mt_path = os.path.join(out_dir, "weekly_ai_news_modal_top.png")
        ...
        time.sleep(1)
        mi_path = os.path.join(out_dir, "weekly_ai_news_modal_inline_images.png")
...
print("Weekly AI news column Playwright verifications passed!", flush=True)
sys.exit(0)
