...
try:
    with sync_playwright() as p:
        ...
        time.sleep(1)
        hp_path = os.path.join(out_dir, "vol5_homepage.png")
        ...
        time.sleep(1)
        cl_path = os.path.join(out_dir, "vol5_columns_list.png")
        page.screenshot(path=cl_path)
        print(f"Columns list screenshot saved to {cl_path}", flush=True)
        
        # Open Vol 5 modal directly via JS
        page.evaluate("openColumnModal('column-blue-calculator-history')")
        page.wait_for_selector("#column-modal.is-open")
        time.sleep(1)
        
        # Top of modal screenshot
        mt_path = os.path.join(out_dir, "vol5_modal_top.png")
        ...
        time.sleep(1)
        mi_path = os.path.join(out_dir, "vol5_modal_inline_images.png")
...
print("All Playwright verifications completed successfully!", flush=True)
sys.exit(0)
