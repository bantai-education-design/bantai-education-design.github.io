...
card=function(row){
  ...
  return `<article class="university-card tokyo-card finished-university-card theme-${finishedTheme(row)}" data-type="${esc(row.establishment_type)}" data-id="${esc(row.id)}" data-card-ready="true">${finishedVisual(row)}<div class="card-body"><div class="finished-card-top"><span class="status-pill ${stopped?'stopped':'active'}">${stopped?'募集停止':'募集継続'}</span><span class="place-pill">東京都 ${esc(municipality(row))}</span></div><h3>${esc(row.name)}</h3><p class="finished-summary">${esc(summary)}</p><div class="finished-facts"><div><small>在籍者数</small><strong>${formatStudents(row)}</strong></div><div><small>学部</small><strong>${f.length?`${f.length}`:'—'}</strong></div><div><small>学科</small><strong>${d.length?`${d.length}`:'—'}</strong></div></div>${fieldHtml}<div class="finished-academic-line"><span>学部 ${f.length||'—'}</span><span>研究科 ${g.length||'—'}</span></div><button class="compare-toggle ${compared?'active':''}" type="button" data-compare="${esc(row.id)}">${compared?'✓ 比較候補に追加済み':'＋ 比較候補に追加'}</button><div class="card-actions">${row.admissions_url?`<a class="btn primary" href="${esc(row.admissions_url)}" target="_blank" rel="noopener">入試情報 ↗</a>`:row.official_url?`<a class="btn primary" href="${esc(row.official_url)}" target="_blank" rel="noopener">公式情報 ↗</a>`:'<span class="btn" aria-disabled="true">公式情報</span>'}<a class="btn map-card-link" href="${mapsUrl(row)}" target="_blank" rel="noopener">地図 ↗</a></div></div></article>`;
};
