...
function installGallery(){
  ...
  if(hero.querySelector('.detail-photo-preview-gallery'))return true;
  ...
  gallery.innerHTML=`<img class="detail-photo-preview-main" src="${preparedPhotos[0].url}" alt="${universityName}の実景写真"><div class="detail-photo-preview-shade"></div><span class="detail-photo-preview-badge">実画面プレビュー</span><div class="detail-photo-preview-thumbs"></div>`;
...
function ensureGallery(){
  ...
  const hero=document.querySelector('.detail-hero');
  if(!hero)return;
  if(!hero.querySelector('.detail-photo-preview-gallery'))installGallery();
}
