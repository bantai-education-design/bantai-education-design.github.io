...
function enhance(){document.querySelectorAll('.tokyo-card').forEach(card=>{if(card.querySelector('.card-detail-open'))return;const actions=card.querySelector('.card-actions');if(!actions)return;const b=document.createElement('button');b.type='button';b.className='btn card-detail-open';b.dataset.detail=card.dataset.id;b.textContent='詳しく見る';b.setAttribute('aria-label',`${card.querySelector('h3')?.textContent||'大学'}の詳細を見る`);b.onclick=e=>{e.preventDefault();e.stopPropagation();openDetail(card.dataset.id,b);};actions.prepend(b);});}
