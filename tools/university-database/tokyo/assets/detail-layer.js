...
function previewVisual(row,photos){
  const ordered=[...photos].sort((a,b)=>(b.main?1:0)-(a.main?1:0)).slice(0,5);
  const main=ordered[0];
  const thumbs=ordered.slice(1).map((photo,index)=>`<button type="button" class="detail-photo-thumb" data-preview-photo="${index+1}" aria-label="写真${index+2}を背景に表示"><img src="${safe(photo.url)}" alt=""></button>`).join('');
  return `<div class="detail-visual has-image detail-photo-gallery"><img class="detail-photo-main" src="${safe(main.url)}" alt="${safe(row.name)}のキャンパス実景"><div class="detail-photo-shade"></div><div class="detail-photo-overlay"><span class="detail-photo-preview-label">実画面プレビュー</span><strong>${safe(row.name)}</strong></div>${thumbs?`<div class="detail-photo-thumbs"><button type="button" class="detail-photo-thumb active" data-preview-photo="0" aria-label="メイン写真を背景に表示"><img src="${safe(main.url)}" alt=""></button>${thumbs}</div>`:''}</div>`;
}
function visual(row){const previews=previewPhotos.get(row.id);if(previews?.length)return previewVisual(row,previews);const image=detailVerifiedImage(row);if(image){const ai=image.rights_status==='ai_original';return`<div class="detail-visual has-image"><img src="${safe(image.image_url)}" alt="${safe(image.alt||`${row.name}の大学紹介イメージ`)}"><span>${ai?safe(image.label||'イメージ画像（AI生成）'):'大学紹介画像'}</span></div>`;}return`<div class="detail-visual themed"><span class="detail-theme-kicker">${safe(themeLabel(row))}</span><strong>${safe(row.name)}</strong><small>学問分野と大学情報をもとにした案内ビジュアル</small></div>`;}
...
function bindPreviewGallery(dialog,row){
  const photos=previewPhotos.get(row.id);if(!photos?.length)return;
  const ordered=[...photos].sort((a,b)=>(b.main?1:0)-(a.main?1:0)).slice(0,5);
  const main=dialog.querySelector('.detail-photo-main');
  dialog.querySelectorAll('.detail-photo-thumb').forEach(btn=>btn.addEventListener('click',()=>{
    const photo=ordered[Number(btn.dataset.previewPhoto)||0];if(!photo||!main)return;
    main.src=photo.url;
    dialog.querySelectorAll('.detail-photo-thumb').forEach(x=>x.classList.toggle('active',x===btn));
  }));
}
