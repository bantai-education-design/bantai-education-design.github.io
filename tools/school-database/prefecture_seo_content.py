...
def build_stats_paragraph(pref_name: str, card_pref: dict, dataset_pref: dict | None = None) -> str:
    ...
    if dataset_pref and "total_population" in dataset_pref:
        ...
        intro = (
            ...
            f"令和2年国勢調査確定値）です。"
            f"このうち3〜17歳の校種相当年齢人口は{format_number(census_age_3_17)}人（総人口に占める割合は{share}%）です。"
        )
    else:
        ...
        intro = (
            f"{pref_name}の総人口は{format_number(census_population)}人、"
            f"このうち3〜17歳の校種相当年齢人口は{format_number(census_age_3_17)}人（{share}%）です"
...
def build_faq_items(pref_name: str, total: int, municipality_count: int, school_type_count: int) -> list[dict]:
    ...
    return [
        ...
        {
            "question": "市区町村ごとに学校を探せますか？",
            "answer": f"はい、{pref_name}内の市区町村を選択して、その地域の学校だけを一覧表示できます。",
        },
        {
        ...
        {
            "question": "学校の電話番号を一覧で確認できますか？",
            "answer": "はい、検索結果には各学校の電話番号を表示しており、CSVダウンロードにも電話番号を含めています。",
        },
        {
            "question": "学校住所をCSVで保存できますか？",
            "answer": "はい、検索結果の学校名・住所・郵便番号・電話番号をCSV形式でダウンロードできます。",
        },
        {
            "question": "学校へ郵送する宛名を作成できますか？",
            "answer": (
                "はい、検索結果から「校長先生宛て」「御中」など敬称を選んで宛名をコピーでき、"
                "封筒印刷にそのままお使いいただけます。"
            ),
        },
        {
