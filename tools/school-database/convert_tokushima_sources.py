...
def normalize_address(value: Any) -> str:
    ...
    if text and not text.startswith("徳島県"):
        text = "徳島県" + text
    return text
...
FUKUI_CITIES = [
    "あわら市", "勝山市", "坂井市", "大野市", "小浜市", "敦賀市", "徳島市", "越前市", "鯖江市"
]
...
for full in FUKUI_GUN_TOWNS:
    ...
    if m:
        bare = m.group(1)
        # 同じ bare 名が複数の郡に存在しないことを前提（徳島県は問題なし）
...
def infer_municipality(address: str) -> str:
    text = address
    for pref_name in ["富山県", "徳島県", "愛媛県", "高知県", "徳島県", "石川県", "滋賀県", "岐阜県", "三重県", "兵庫県", "岡山県", "山口県", "広島県", "鳥取県", "島根県"]:
...
def build_official_name(*, establishment: str, raw_name: str,
    ...
    # 例: 「山形市立商業高等学校」「徳島県立致道館中学校」
    # 「徳島県立村山特別支援学校山形校」（分校も含む）
...
def make_record(
    *, name: str, name_kana: str, postal_code: str, address: str,
    ...
    return {
        "id": f"tokushima-{slug(stable_key)}",
        "prefecture": "徳島県",
        "name": name,
...
def main() -> int:
    ...
    parser.add_argument("--output", type=Path, default=Path("data/school-database/tokushima.json"))
    parser.add_argument("--warnings-output", type=Path,
                        default=Path("tools/school-database/tokushima_conversion_warnings.json"))
