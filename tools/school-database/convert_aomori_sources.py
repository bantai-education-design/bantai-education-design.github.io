...
def normalize_address(value: Any) -> str:
    ...
    if text and not text.startswith("青森県"):
        text = "青森県" + text
    return text
...
MUNICIPALITY_ORDER = AOMORI_CITIES + AOMORI_GUN_TOWNS

# 郡名なし → 正式名称へのマッピング（分詞が重なる村名・町名の解決）
_BARE_TO_CANONICAL: dict[str, str] = {}
for full in AOMORI_GUN_TOWNS:
    m = re.match(r"^.+郡(.+)$", full)
    if m:
        bare = m.group(1)
        # 同じ bare 名が複数の郡に存在しないことを前提（青森県は問題なし）
...
_AOMORI_MUNI_NAMES = {re.sub(r"^.+郡", "", m) for m in MUNICIPALITY_ORDER}
_AOMORI_MUNI_NAMES.update(AOMORI_CITIES)
...
def build_official_name(*, establishment: str, raw_name: str,
    ...
    2. MEXT名称が「○○市立△△高等学校」のように設置者を含む場合は
       「青森県立」を重ねて付けない（市立商業高等学校等）
    3. 分校名（「〜青森校」「〜金山校」等）は「立」を含む完全な固有名称
       → 校種サフィックスを追加しない
    """
    ...
    for mname in _AOMORI_MUNI_NAMES:
...
def load_mext_data(source_root: Path) -> list[dict[str, Any]]:
    ...
    candidates = [
        source_root / "sc_20260529-mxt_chousa01-000011635_1.xlsx",
        Path("data-source/tochigi") / "sc_20260529-mxt_chousa01-000011635_1.xlsx",
    ]
    excel_path = None
    for c in candidates:
        if c.exists():
...
def main() -> int:
    ...
    parser.add_argument("--output", type=Path, default=Path("data/school-database/aomori.json"))
    parser.add_argument("--warnings-output", type=Path,
                        default=Path("tools/school-database/aomori_conversion_warnings.json"))
