...
def normalize_address(value: Any) -> str:
    ...
    if text and not text.startswith("秋田県"):
        text = "秋田県" + text
    return text
...
MUNICIPALITY_ORDER = AKITA_CITIES + AKITA_GUN_TOWNS

# 郡名なし → 正式名称へのマッピング（分詞が重なる村名・町名の解決）
_BARE_TO_CANONICAL: dict[str, str] = {}
for full in AKITA_GUN_TOWNS:
    m = re.match(r"^.+郡(.+)$", full)
    if m:
        bare = m.group(1)
        # 同じ bare 名が複数の郡に存在しないことを前提（秋田県は問題なし）
...
_AKITA_MUNI_NAMES = {re.sub(r"^.+郡", "", m) for m in MUNICIPALITY_ORDER}
_AKITA_MUNI_NAMES.update(AKITA_CITIES)
...
def build_official_name(*, establishment: str, raw_name: str,
    ...
    2. MEXT名称が「○○市立△△高等学校」のように設置者を含む場合は
       「秋田県立」を重ねて付けない（市立商業高等学校等）
    3. 分校名（「〜秋田校」「〜金山校」等）は「立」を含む完全な固有名称
       → 校種サフィックスを追加しない
    """
    ...
    for mname in _AKITA_MUNI_NAMES:
...
def main() -> int:
    ...
    parser.add_argument("--output", type=Path, default=Path("data/school-database/akita.json"))
    parser.add_argument("--warnings-output", type=Path,
                        default=Path("tools/school-database/akita_conversion_warnings.json"))
