...
def normalize_address(value: Any) -> str:
    ...
    if text and not text.startswith("岡山県"):
        text = "岡山県" + text
    return text
...
OKAYAMA_CITIES = ['岡山市', '倉敷市', '津山市', '玉野市', '笠岡市', '井原市', '総社市', '高梁市', '新見市', '備前市', '瀬戸内市', '赤磐市', '真庭市', '美作市', '浅口市']

OKAYAMA_GUN_TOWNS = ['和気郡和気町', '都窪郡早島町', '浅口郡里庄町', '小田郡矢掛町', '真庭郡新庄村', '苫田郡鏡野町', '勝田郡勝央町', '勝田郡奈義町', '英田郡西粟倉村', '久米郡久米南町', '久米郡美咲町', '加賀郡吉備中央町']

MUNICIPALITY_ORDER = OKAYAMA_CITIES + OKAYAMA_GUN_TOWNS

# 郡名なし → 正式名称へのマッピング（分詞が重なる村名・町名の解決）
_BARE_TO_CANONICAL: dict[str, str] = {}
for full in OKAYAMA_GUN_TOWNS:
    m = re.match(r"^.+郡(.+)$", full)
    if m:
        bare = m.group(1)
        # 同じ bare 名が複数の郡に存在しないことを前提（岡山県は問題なし）
...
_OKAYAMA_MUNI_NAMES = {re.sub(r"^.+郡", "", m) for m in MUNICIPALITY_ORDER}
_OKAYAMA_MUNI_NAMES.update(OKAYAMA_CITIES)
...
def build_official_name(*, establishment: str, raw_name: str,
    ...
    # 例: 「山形市立商業高等学校」「岡山県立致道館中学校」
    # 「岡山県立村山特別支援学校山形校」（分校も含む）
    ...
    for mname in _OKAYAMA_MUNI_NAMES:
...
def main() -> int:
    ...
    parser.add_argument("--output", type=Path, default=Path("data/school-database/okayama.json"))
    parser.add_argument("--warnings-output", type=Path,
                        default=Path("tools/school-database/okayama_conversion_warnings.json"))
