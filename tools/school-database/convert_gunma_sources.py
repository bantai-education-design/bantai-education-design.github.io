...
def normalize_address(value: Any) -> str:
    ...
    if text and not text.startswith("群馬県"):
        text = "群馬県" + text
    return text
...
MUNICIPALITY_ORDER = GUNMA_CITIES + GUNMA_GUN_TOWNS

# 郡名なし → 正式名称へのマッピング（分詞が重なる村名・町名の解決）
_BARE_TO_CANONICAL: dict[str, str] = {}
for full in GUNMA_GUN_TOWNS:
    m = re.match(r"^.+郡(.+)$", full)
    if m:
        bare = m.group(1)
        # 同じ bare 名が複数の郡に存在しないことを前提（群馬県は問題なし）
...
_GUNMA_MUNI_NAMES = {re.sub(r"^.+郡", "", m) for m in MUNICIPALITY_ORDER}
_GUNMA_MUNI_NAMES.update(GUNMA_CITIES)
...
def build_official_name(*, establishment: str, raw_name: str,
    ...
    for mname in _GUNMA_MUNI_NAMES:
...
def load_mext_data(source_root: Path) -> list[dict[str, Any]]:
    ...
    yama = df[df["都道府県番号"].fillna("").str.startswith("10(")].copy()
...
def main() -> int:
    ...
    parser.add_argument("--output", type=Path, default=Path("data/school-database/gunma.json"))
    parser.add_argument("--warnings-output", type=Path,
                        default=Path("tools/school-database/gunma_conversion_warnings.json"))
