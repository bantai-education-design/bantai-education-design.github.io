...
def normalize_address(value: Any) -> str:
    ...
    if text and not text.startswith("岐阜県"):
        text = "岐阜県" + text
    return text
...
GIFU_GUN_TOWNS = [
    "羽島郡岐南町", "羽島郡笠松町", "養老郡養老町", "不破郡垂井町", "不破郡関ケ原町",
    "安八郡神戸町", "安八郡輪之内町", "安八郡安八町", "揖斐郡揖斐川町", "揖斐郡大野町", "揖斐郡池田町",
    "本巣郡北方町", "加茂郡坂祝町", "加茂郡富加町", "加茂郡川辺町", "加茂郡七宗町", "加茂郡八百津町",
    "加茂郡白川町", "加茂郡東白川村", "可児郡御嵩町", "大野郡白川村"
]

MUNICIPALITY_ORDER = GIFU_CITIES + GIFU_GUN_TOWNS

# 郡名なし → 正式名称へのマッピング（分詞が重なる村名・町名の解決）
_BARE_TO_CANONICAL: dict[str, str] = {}
for full in GIFU_GUN_TOWNS:
    m = re.match(r"^.+郡(.+)$", full)
    if m:
        bare = m.group(1)
        # 同じ bare 名が複数の郡に存在しないことを前提（岐阜県は問題なし）
...
_GIFU_MUNI_NAMES = {re.sub(r"^.+郡", "", m) for m in MUNICIPALITY_ORDER}
_GIFU_MUNI_NAMES.update(GIFU_CITIES)
...
def main() -> int:
    ...
    parser.add_argument("--output", type=Path, default=Path("data/school-database/gifu.json"))
    parser.add_argument("--warnings-output", type=Path,
                        default=Path("tools/school-database/gifu_conversion_warnings.json"))
