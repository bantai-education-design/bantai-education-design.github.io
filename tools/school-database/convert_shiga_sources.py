...
def normalize_address(value: Any) -> str:
    ...
    if text and not text.startswith("滋賀県"):
        text = "滋賀県" + text
    return text
...
SHIGA_CITIES = [
    "大津市", "守山市", "彦根市", "東近江市", "栗東市", "湖南市", "甲賀市", "米原市", "草津市", "近江八幡市", "野洲市", "長浜市", "高島市"
]

SHIGA_GUN_TOWNS = [
    "愛知郡愛荘町", "愛知郡愛荘町市", "犬上郡多賀町", "犬上郡甲良町", "犬上郡豊郷町", "蒲生郡日野町", "蒲生郡竜王町"
]

MUNICIPALITY_ORDER = SHIGA_CITIES + SHIGA_GUN_TOWNS

# 郡名なし → 正式名称へのマッピング（分詞が重なる村名・町名の解決）
_BARE_TO_CANONICAL: dict[str, str] = {}
for full in SHIGA_GUN_TOWNS:
    m = re.match(r"^.+郡(.+)$", full)
    if m:
        bare = m.group(1)
        # 同じ bare 名が複数の郡に存在しないことを前提（滋賀県は問題なし）
...
_SHIGA_MUNI_NAMES = {re.sub(r"^.+郡", "", m) for m in MUNICIPALITY_ORDER}
_SHIGA_MUNI_NAMES.update(SHIGA_CITIES)
...
def build_official_name(*, establishment: str, raw_name: str,
    ...
    # 例: 「山形市立商業高等学校」「滋賀県立致道館中学校」
    # 「滋賀県立村山特別支援学校山形校」（分校も含む）
    ...
    for mname in _SHIGA_MUNI_NAMES:
...
def main() -> int:
    ...
    parser.add_argument("--output", type=Path, default=Path("data/school-database/shiga.json"))
    parser.add_argument("--warnings-output", type=Path,
                        default=Path("tools/school-database/shiga_conversion_warnings.json"))
