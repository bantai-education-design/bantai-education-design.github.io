...
def normalize_address(value: Any) -> str:
    ...
    if text and not text.startswith("京都府"):
        text = "京都府" + text
    return text
...
for full in YAMAGATA_GUN_TOWNS:
    ...
    if m:
        bare = m.group(1)
        # 同じ bare 名が複数の郡に存在しないことを前提（京都府は問題なし）
...
def main() -> int:
    ...
    parser.add_argument("--output", type=Path, default=Path("data/school-database/kyoto.json"))
    parser.add_argument("--warnings-output", type=Path,
                        default=Path("tools/school-database/kyoto_conversion_warnings.json"))
