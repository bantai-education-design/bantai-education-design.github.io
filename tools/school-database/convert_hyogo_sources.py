...
def normalize_address(value: Any) -> str:
    ...
    if text and not text.startswith("兵庫県"):
        text = "兵庫県" + text
    return text
...
HYOGO_CITIES = ['神戸市', '姫路市', '尼崎市', '明石市', '西宮市', '洲本市', '芦屋市', '伊丹市', '相生市', '豊岡市', '加古川市', '赤穂市', '西脇市', '宝塚市', '三木市', '高砂市', '川西市', '小野市', '三田市', '加西市', '丹波篠山市', '養父市', '丹波市', '南あわじ市', '朝来市', '淡路市', '宍粟市', '加東市', 'たつの市']

HYOGO_GUN_TOWNS = ['川辺郡猪名川町', '多可郡多可町', '加古郡稲美町', '加古郡播磨町', '神崎郡市川町', '神崎郡福崎町', '神崎郡神河町', '揖保郡太子町', '赤穂郡上郡町', '佐用郡佐用町', '美方郡香美町', '美方郡新温泉町']

MUNICIPALITY_ORDER = HYOGO_CITIES + HYOGO_GUN_TOWNS

# 郡名なし → 正式名称へのマッピング（分詞が重なる村名・町名の解決）
_BARE_TO_CANONICAL: dict[str, str] = {}
for full in HYOGO_GUN_TOWNS:
    m = re.match(r"^.+郡(.+)$", full)
    if m:
        bare = m.group(1)
        # 同じ bare 名が複数の郡に存在しないことを前提（兵庫県は問題なし）
...
_HYOGO_MUNI_NAMES = {re.sub(r"^.+郡", "", m) for m in MUNICIPALITY_ORDER}
_HYOGO_MUNI_NAMES.update(HYOGO_CITIES)
...
def build_official_name(*, establishment: str, raw_name: str,
    ...
    # 例: 「山形市立商業高等学校」「兵庫県立致道館中学校」
    # 「兵庫県立村山特別支援学校山形校」（分校も含む）
    ...
    for mname in _HYOGO_MUNI_NAMES:
...
def main() -> int:
    ...
    parser.add_argument("--output", type=Path, default=Path("data/school-database/hyogo.json"))
    parser.add_argument("--warnings-output", type=Path,
                        default=Path("tools/school-database/hyogo_conversion_warnings.json"))
