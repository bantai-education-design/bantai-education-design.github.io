...
def normalize_address(value: Any) -> str:
    ...
    if text and not text.startswith("熊本県"):
        text = "熊本県" + text
    return text
...
KUMAMOTO_CITIES = [
    "上天草市", "人吉市", "八代市", "合志市", "天草市", "宇土市", "宇城市", "山鹿市", "水俣市", "熊本市", "玉名市", "荒尾市", "菊池市", "阿蘇市"
]

KUMAMOTO_GUN_TOWNS = [
    "上益城郡嘉島町", "上益城郡山都町", "上益城郡御船町", "上益城郡甲佐町", "上益城郡益城町", "下益城郡美里町", "八代郡氷川町", "天草郡苓北町", "玉名郡南関町", "玉名郡和水町", "玉名郡玉東町", "玉名郡長洲町", "球磨郡あさぎり町", "球磨郡五木村", "球磨郡多良木町", "球磨郡山江村", "球磨郡水上村", "球磨郡湯前町", "球磨郡球磨村", "球磨郡相良村", "球磨郡錦町", "菊池郡大津町", "菊池郡菊陽町", "葦北郡津奈木町", "葦北郡芦北町", "葦北郡芦北町市", "阿蘇郡南小国町", "阿蘇郡南阿蘇村", "阿蘇郡小国町", "阿蘇郡産山村", "阿蘇郡西原村", "阿蘇郡高森町"
]

MUNICIPALITY_ORDER = KUMAMOTO_CITIES + KUMAMOTO_GUN_TOWNS

# 郡名なし → 正式名称へのマッピング（分詞が重なる村名・町名の解決）
_BARE_TO_CANONICAL: dict[str, str] = {}
for full in KUMAMOTO_GUN_TOWNS:
    m = re.match(r"^.+郡(.+)$", full)
    if m:
        bare = m.group(1)
        # 同じ bare 名が複数の郡に存在しないことを前提（熊本県は問題なし）
...
_KUMAMOTO_MUNI_NAMES = {re.sub(r"^.+郡", "", m) for m in MUNICIPALITY_ORDER}
_KUMAMOTO_MUNI_NAMES.update(KUMAMOTO_CITIES)
...
def build_official_name(*, establishment: str, raw_name: str,
    ...
    # 例: 「山形市立商業高等学校」「熊本県立致道館中学校」
    # 「熊本県立村山特別支援学校山形校」（分校も含む）
    ...
    for mname in _KUMAMOTO_MUNI_NAMES:
...
def main() -> int:
    ...
    parser.add_argument("--output", type=Path, default=Path("data/school-database/kumamoto.json"))
    parser.add_argument("--warnings-output", type=Path,
                        default=Path("tools/school-database/kumamoto_conversion_warnings.json"))
