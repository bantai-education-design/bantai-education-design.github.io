...
原本: 文部科学省「学校コード一覧（全国公立・国立・私立学校等）」
      sc_20260529-mxt_chousa01-000011635_4.csv（2026-05-29版）
...
def normalize_address(value: Any) -> str:
    ...
    if text and not text.startswith("福岡県"):
        text = "福岡県" + text
    return text
...
MUNICIPALITY_ORDER = FUKUOKA_CITIES + FUKUOKA_GUN_TOWNS

# 郡名なし → 正式名称へのマッピング（分詞が重なる村名・町名の解決）
_BARE_TO_CANONICAL: dict[str, str] = {}
for full in FUKUOKA_GUN_TOWNS:
    m = re.match(r"^.+郡(.+)$", full)
    if m:
        bare = m.group(1)
        # 同じ bare 名が複数の郡に存在しないことを前提（福岡県は問題なし）
...
_FUKUOKA_MUNI_NAMES = {re.sub(r"^.+郡", "", m) for m in MUNICIPALITY_ORDER}
_FUKUOKA_MUNI_NAMES.update(FUKUOKA_CITIES)
...
def build_official_name(*, establishment: str, raw_name: str,
    ...
    2. MEXT名称が「○○市立△△高等学校」のように設置者を含む場合は
       「福岡県立」を重ねて付けない（市立商業高等学校等）
    3. 分校名（「〜福岡校」「〜金山校」等）は「立」を含む完全な固有名称
       → 校種サフィックスを追加しない
    """
    ...
    if school_type == "特別支援学校" and "特別支援学校" in raw_name:
    ...
    for mname in _FUKUOKA_MUNI_NAMES:
...
def load_mext_data(source_root: Path) -> list[dict[str, Any]]:
    ...
    candidates = [
        source_root / "sc_20260529-mxt_chousa01-000011635_4.csv",
        Path("data-source/tochigi") / "sc_20260529-mxt_chousa01-000011635_4.csv",
    ]
    excel_path = None
    for c in candidates:
        if c.exists():
...
def main() -> int:
    ...
    parser.add_argument("--output", type=Path, default=Path("data/school-database/fukuoka.json"))
    parser.add_argument("--warnings-output", type=Path,
                        default=Path("tools/school-database/fukuoka_conversion_warnings.json"))
