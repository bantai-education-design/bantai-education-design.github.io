...
def test_all_47_have_seo_content_section() -> None:
    ...
    for meta in prefecture_metadata:
        ...
        assert block.count('<details class="pref-faq-item">') == 7, f"{slug}: FAQ件数が7件ではありません"
...
def test_faq_visible_content_matches_json_ld_exactly() -> None:
    ...
    for meta in prefecture_metadata:
        ...
        assert len(visible_faq) == 7, f"{slug}: 可視FAQが7件見つかりません（{len(visible_faq)}件）"
        ...
        assert len(faq_page["mainEntity"]) == 7, f"{slug}: JSON-LDのFAQ件数が7件ではありません"
