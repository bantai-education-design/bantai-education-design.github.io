...
def normalize_address(value: Any) -> str:
    ...
    if text and not text.startswith("山口県"):
        text = "山口県" + text
    return text
...
YAMAGUCHI_CITIES = ['下関市', '宇部市', '山口市', '萩市', '防府市', '下松市', '岩国市', '光市', '長門市', '柳井市', '美祢市', '周南市', '山陽小野田市']

YAMAGUCHI_GUN_TOWNS = ['大島郡周防大島町', '玖珂郡和木町', '熊毛郡上関町', '熊毛郡田布施町', '熊毛郡平生町', '阿武郡阿武町']

MUNICIPALITY_ORDER = YAMAGUCHI_CITIES + YAMAGUCHI_GUN_TOWNS

# 郡名なし → 正式名称へのマッピング（分詞が重なる村名・町名の解決）
_BARE_TO_CANONICAL: dict[str, str] = {}
for full in YAMAGUCHI_GUN_TOWNS:
    m = re.match(r"^.+郡(.+)$", full)
    if m:
        bare = m.group(1)
        # 同じ bare 名が複数の郡に存在しないことを前提（山口県は問題なし）
...
_YAMAGUCHI_MUNI_NAMES = {re.sub(r"^.+郡", "", m) for m in MUNICIPALITY_ORDER}
_YAMAGUCHI_MUNI_NAMES.update(YAMAGUCHI_CITIES)
...
def build_official_name(*, establishment: str, raw_name: str,
    ...
    # 例: 「山形市立商業高等学校」「山口県立致道館中学校」
    # 「山口県立村山特別支援学校山形校」（分校も含む）
    ...
    for mname in _YAMAGUCHI_MUNI_NAMES:
...
def make_record(
    *, name: str, name_kana: str, postal_code: str, address: str,
    ...
    return {
        "id": f"yamaguchi-{slug(stable_key)}",
        "prefecture": "山口県",
        "name": name,
...
def main() -> int:
    ...
    parser.add_argument("--output", type=Path, default=Path("data/school-database/yamaguchi.json"))
    parser.add_argument("--warnings-output", type=Path,
                        default=Path("tools/school-database/yamaguchi_conversion_warnings.json"))
