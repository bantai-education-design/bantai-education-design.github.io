...
def normalize_address(value: Any) -> str:
    ...
    if text and not text.startswith("佐賀県"):
        text = "佐賀県" + text
    return text
...
SAGA_CITIES = [
    "伊万里市", "佐賀市", "唐津市", "多久市", "嬉野市", "小城市", "武雄市", "神埼市", "鳥栖市", "鹿島市"
]

SAGA_GUN_TOWNS = [
    "三養基郡みやき町", "三養基郡みやき町大字市", "三養基郡みやき町市", "三養基郡上峰町", "三養基郡基山町", "東松浦郡玄海町", "杵島郡大町", "杵島郡江北町", "杵島郡白石町", "神埼郡吉野ヶ里町", "藤津郡太良町", "西松浦郡有田町"
]

MUNICIPALITY_ORDER = SAGA_CITIES + SAGA_GUN_TOWNS

# 郡名なし → 正式名称へのマッピング（分詞が重なる村名・町名の解決）
_BARE_TO_CANONICAL: dict[str, str] = {}
for full in SAGA_GUN_TOWNS:
    m = re.match(r"^.+郡(.+)$", full)
    if m:
        bare = m.group(1)
        # 同じ bare 名が複数の郡に存在しないことを前提（佐賀県は問題なし）
...
_SAGA_MUNI_NAMES = {re.sub(r"^.+郡", "", m) for m in MUNICIPALITY_ORDER}
_SAGA_MUNI_NAMES.update(SAGA_CITIES)
...
def build_official_name(*, establishment: str, raw_name: str,
    ...
    # 例: 「山形市立商業高等学校」「佐賀県立致道館中学校」
    # 「佐賀県立村山特別支援学校山形校」（分校も含む）
...
def main() -> int:
    ...
    parser.add_argument("--output", type=Path, default=Path("data/school-database/saga.json"))
    parser.add_argument("--warnings-output", type=Path,
                        default=Path("tools/school-database/saga_conversion_warnings.json"))
