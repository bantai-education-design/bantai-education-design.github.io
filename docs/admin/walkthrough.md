...
| **Commit 5** | 本コミット | `fix(analytics): unify admin exclusion parameter to bantai_admin and add event verification tests`<br>- 除外URLパラメータを `?bantai_admin=true` / `?bantai_admin=clear` に完全統一<br>- 全公開88HTML、JS、仕様書、手順書の表記統一<br>- イベント発火・サニタイズ・二重送信防止の自動検証テスト `tools/test_analytics_events.js` の追加 |
...
### 3.2 自動化テスト
- `node tools/test_analytics_events.js`: イベント発火、サニタイズ、除外判定、コラム滞在時間ロジック全件合格（PASS）
- `python tools/audit_html_ga4.py`: 98HTML全件の分類・タグ設置チェック通過（PASS）
