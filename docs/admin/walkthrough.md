...
| **Commit 6** | `a9ba384` | `fix(analytics): add read_time_sec metric, address bar replaceState cleanup, and refine realtime verification guides`<br>- `assets/js/columns.js` に `read_time_sec` を追加し、GA4カスタム指標として仕様書・レポート手順書に整備<br>- 全88公開HTMLのPre-config Inline Exclusionスクリプトに `window.history.replaceState` によるアドレスバー・閲覧履歴クエリ消去処理を実装<br>- `docs/admin/admin-exclusion-guide.md` の確認手順を整理（リアルタイムレポートを第一選択、DebugViewをTag Assistant併用の詳細検証用として位置づけ）<br>- `tools/audit_html_ga4.py` および `tools/test_analytics_events.js` を拡張し自動検証 |
| **Commit 7** | 本コミット | `fix(ci): trigger owner-overlap guard on all pull requests to satisfy main branch protection`<br>- `.github/workflows/university-photo-owner-overlap-guard.yml` の `paths:` 制限を解除<br>- mainブランチの保護ルールで必須指定されている `owner-overlap` チェックが全PRで確実に起動・結果報告されるよう恒久修正 |
