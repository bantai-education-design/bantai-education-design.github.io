        ...
        with urllib.request.urlopen(req, timeout=10) as res:
            ...
            for href, text in matches:
                ...
                if any(ext in full.lower() for ext in ('.xlsx', '.xls', '.pdf', '.csv', '.zip')):
                    ...
                    # Auto-download interesting Excel files or main school lists
                    if any(kw in clean_text or kw in full for kw in ('一覧', '名簿', '学校', '小', '中', '高', '特別', '公立', '私立', '結果', '統', '表', 'r7', 'r6', '2025', '2026')):
                        fname = os.path.basename(full).split('?')[0]
                        out_path = os.path.join(os.path.dirname(__file__), f"dl_{fname}")
                        try:
                ...
                elif any(kw in clean_text for kw in ('令和', 'R7', 'R6', '報告書', '一覧', '学校表', '結果', '統計', '詳細')):
