...
def generate_script(pref_en, pref_ja, pref_suffix, pref_code_str, mext_source):
    cities = fetch_cities(f"{pref_ja}{pref_suffix}")
    script = template
    ...
    # Update hardcoded prefecture names in address inference
    script = script.replace('山形県', f'{pref_ja}{pref_suffix}')
...
def infer_municipality(address: str) -> str:
    text = address
    if text.startswith("{pref_ja}{pref_suffix}"):
        text = text[len("{pref_ja}{pref_suffix}"):]
...
generate_script("kyoto", "京都", "府", "26", "sc_20260529-mxt_chousa01-000011635_3.xlsx")
generate_script("nara", "奈良", "県", "29", "sc_20260529-mxt_chousa01-000011635_3.xlsx")
